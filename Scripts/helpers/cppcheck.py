...
def build_cppcheck_command(
    ...
    """
    del max_parallel_jobs  # -j is intentionally unused; see note below.

    # Only scan include/ if it exists (a fresh checkout always has it, but
    # keeps this robust if the layout is ever restructured).
    scan_dirs = [_SOURCE_ROOT] if os.path.isdir(os.path.join(source_path, _SOURCE_ROOT)) else []
