#!/usr/bin/env python3
"""Serialization CMake Build Configuration Script.

Design follows KhwarizmiAnalytix/XSigma's Scripts/setup.py (dotted-token CLI,
a Flags/Configuration class split, coverage-tool integration), scaled down
the way KhwarizmiAnalytix/Parallel's Scripts/setup.py already scaled it down,
to the CMake options this standalone repo actually defines
(SERIALIZATION_ENABLE_*/SERIALIZATION_* across CMakeLists.txt and
cmake/*.cmake). There is no multi-module Library/* tree, no --project.*
scoping, and no GPU/MKL/vectorization backends here.

Unlike Parallel, Serialization has no CMake option for testing, GoogleTest,
benchmarks, examples, or sanitizers at all: CTest is unconditionally enabled
and GoogleTest is unconditionally fetched via FetchContent in CMakeLists.txt,
so this script's "test" token is purely a workflow step (run ctest after the
build), not a CMake flag like Parallel's PARALLEL_ENABLE_TESTING.
...
class ErrorLogger:
    ...
    def __init__(self, log_dir: str = "logs"):
        ...
        self.log_file = (
            self.log_dir / f"serialization_build_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
        self.errors = []
...
class SerializationFlags:
    """Maps setup.py dotted tokens to Serialization's SERIALIZATION_* CMake cache variables.

    Scoped 1:1 to the options CMakeLists.txt and cmake/*.cmake actually
    define. Unlike XSigma's XSigmaFlags (or even Parallel's ParallelFlags)
    there is no multi-module fan-out, no GPU/MKL/vectorization backend, no
    --project.* scoping, and — notably — no testing/gtest/benchmark/examples/
    sanitizer toggle of any kind: CTest and GoogleTest are unconditional in
    CMakeLists.txt, so there is simply no CMake flag for them to map.
    """
    ...
    def __initialize_flags(self):
        ...
        self.__description = [
            ...
            "enable spell checking with automatic corrections (WARNING: modifies source files)",
    ...
    def __build_cmake_flag(self):
        ...
        self.__name = {
            "icecc": "SERIALIZATION_ENABLE_ICECC",
            "cache": "SERIALIZATION_ENABLE_CACHE",
            "cache_type": "SERIALIZATION_CACHE_TYPE",
            "clangtidy": "SERIALIZATION_ENABLE_CLANGTIDY",
            "fix": "SERIALIZATION_ENABLE_FIX",
            "spell": "SERIALIZATION_ENABLE_SPELL",
            "coverage": "SERIALIZATION_ENABLE_COVERAGE",
    ...
    def __set_default_flags(self):
        # Mirrors CMakeLists.txt's/cmake/*.cmake's option() defaults exactly.
        self.__value = dict.fromkeys(self.__key, self.OFF)
        self.__value.update(
            {
                "cache": self.ON,  # SERIALIZATION_ENABLE_CACHE default ON
                "cache_type": "none",
            }
        )
    ...
    def create_cmake_flags(self, cmake_cmd_flags, build_enum, system):
        ...
        if self.__value.get("coverage") == self.ON:
            print_status("Enabling debug build for coverage analysis", "INFO")
            # Title-case to match $<CONFIG> generator-expression output used
            # in CMakeLists.txt for the library output directory -- coverage-
            # tool's multi_config_search_dirs() looks for "Debug" (exact
            # case), not "DEBUG".
            build_type = "Debug"
        else:
...
class SerializationConfiguration:
    def __init__(self, args_list):
        ...
        self.__serialization_flags = SerializationFlags(args_list)
    ...
    def __set_ninja_flags(self):
        ...
        self.__value["build_folder"] = f"build_ninja{self.__serialization_flags.builder_suffix}"
    ...
    def __set_xcode_flags(self):
        if self.__value["system"] == "Darwin" and check_xcode_availability():
            ...
            self.__value["build_folder"] = f"build_xcode{self.__serialization_flags.builder_suffix}"
    ...
    def __set_visual_studio(self, arg):
        ...
        self.__value["build_folder"] = f"{base_build_folder}{self.__serialization_flags.builder_suffix}"
    ...
    def config(self, source_path, build_path):
        ...
        try:
            cmake_flags = []
            self.__value["build_enum"] = self.__serialization_flags.create_cmake_flags(
    ...
    def cppcheck(self, source_path, build_path):
        if self.__value["build"] != "build" or not self.__serialization_flags.is_cppcheck():
            return 0
    ...
    def test(self, source_path, build_path):
        del source_path, build_path  # ctest is run from the build directory only
    ...
    def coverage(self, source_path, build_path):
        if self.__value["build"] != "build" or not self.__serialization_flags.is_coverage():
            return 0
...
def parse_args(args):
    """Parse command line arguments, handling dotted-flag notation."""
    processed_args = []
    for arg in args:
...
def main():
    ...
    try:
        ...
        compilation_calc = SerializationConfiguration(arg_list)
