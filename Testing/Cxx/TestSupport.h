...
template <class F>
void expect_error(serialization::error_code code, F&& operation)
{
    try
    {
        operation();
...
public:
    ...
    {
        return number_ == other.number_ && text_ == other.text_;
    }
...
{
public:
    static std::unique_ptr<PrivateRecord> make(int n)
    {
        return std::unique_ptr<PrivateRecord>(new PrivateRecord(n));
    }
