...
inline const adapters::json& json_member(const adapters::json& node, const char* key)
{
    ...
    if (it == node.end())
        throw serialization_error(
            error_code::missing_field, std::string("Missing JSON member: ") + key);
    return *it;
}
template <class T>
void json_scalar(const adapters::json& node, T& value)
{
    ...
    else if constexpr (std::is_integral_v<T>)
    {
        ...
        if (node.is_number_unsigned())
            value = checked_integer<T>(node.get<std::uint64_t>());
        else
            value = checked_integer<T>(node.get<std::int64_t>());
    }
...
{
template <>
struct archive_traits<adapters::json_writer>
{
    ...
    static void write_scalar(A& a, T value)
    {
        if constexpr (std::is_floating_point_v<T>)
            a.node = adapter_detail::checked_float<double>(value);
        else
            a.node = value;
    }
    ...
    static void write_object(A& a, const object_header& h, F&& body)
    {
        ...
        A fields{a.node["$fields"]};
        body(fields);
    }
    template <class F>
    static void write_field(A& a, field_key key, F&& body)
    {
        A child{a.node[std::string(key.name)]};
        body(child);
    }
    template <class F>
    static void write_sequence(A& a, std::size_t, F&& body)
    {
        a.node = json::array();
        body(a);
    }
    ...
    {
        a.node = json{
            {"$kind", static_cast<unsigned>(h.kind)},
            {"$present", h.present},
            {"$index", h.index},
            {"$type", h.type_id}};
        if (h.present)
        {
            A child{a.node["$value"]};
            body(child);
        }
    }
};
...
template <>
struct archive_traits<adapters::json_reader>
{
    using A = adapters::json_reader;
    template <class T>
        requires((std::is_arithmetic_v<T> && sizeof(T) <= 8) || std::is_same_v<T, std::string>)
    static void read_scalar(A& a, T& value)
    {
        adapter_detail::json_scalar(a.node, value);
    }
    template <class F>
    ...
    static void read_tagged(A& a, tag_kind expected, F&& body)
    {
        ...
        adapter_detail::require_json(
            kind == static_cast<unsigned>(expected), "Unexpected JSON tag kind");
        h.kind = expected;
        ...
        if (h.present)
        {
            A child{adapter_detail::json_member(a.node, "$value")};
            body(child, h);
        }
        else
            body(a, h);
