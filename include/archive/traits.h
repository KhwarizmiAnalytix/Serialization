...
concept InputArchive = requires(A& a, object_header o, field_key f) {
    ...
    {
        archive_traits<A>::read_field(a, f, protocol_detail::body{})
    } -> std::same_as<field_presence>;
...
concept ScalarOutput =
    requires(A& a, const T& value) { archive_traits<A>::write_scalar(a, value); };
template <class A, class T>
concept ScalarInput = requires(A& a, T& value) { archive_traits<A>::read_scalar(a, value); };
}  // namespace serialization
