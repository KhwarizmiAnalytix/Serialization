...
template<class F> struct on_exit
{
    F f;
    ~on_exit() { f(); }
};
template<class F> on_exit(F) -> on_exit<F>;
template<class T> constexpr bool builtin_codec = requires { typename type_codec<T>::builtin; };
template<class T> constexpr bool primitive = std::is_arithmetic_v<T> || std::is_enum_v<T> ||
    std::is_same_v<T, std::string>;
}

template<class A, class Metadata, class Base> class polymorphic_registry;
template<class A, class Metadata = macro_metadata> class context;
...
public:
    ...
    {
        if (!registry.frozen())
            throw serialization_error(error_code::invalid_registry, "Freeze the registry before binding");
    ...
    const polymorphic_registry<A, Metadata, Base>* registry() const
    {
        ...
        return it == registries_.end() ? nullptr :
    ...
    template<class F> decltype(auto) at(std::string component, F&& f)
    {
        ...
        {
            if (e.path().empty()) throw serialization_error(e.code(), e.message(), path_);
            throw;
        }
    }

    template<class F> void with_pointer(const void* address, F&& f)
    {
    ...
    template<class T> void save(A& archive, const T& value) requires OutputArchive<A>
    {
        visit([&] { save_value(archive, value); });
    }

    template<class T> void load(A& archive, T& value) requires InputArchive<A>
    {
        static_assert(!std::is_const_v<T>, "Cannot deserialize into a const value");
        visit([&] { load_value(archive, value); });
    }

    template<class T> void save_element(A& archive, std::size_t i, const T& value)
    {
        at("[" + std::to_string(i) + "]", [&]
        {
    ...
    template<class T> void load_element(A& archive, std::size_t i, T& value)
    {
        at("[" + std::to_string(i) + "]", [&]
        {
...
private:
    template<class F> void visit(F&& f)
    {
        at({}, [&]
        {
            if (depth_ >= options_.max_depth)
                throw serialization_error(error_code::depth_limit, "Maximum serialization depth exceeded");
            ++depth_;
    ...
    template<class T> static constexpr std::size_t field_count()
    {
        std::size_t count = 0;
        std::apply([&](const auto&... fields)
        {
            ((count += requires { fields.member(); } ? 1 : 0), ...);
        }, Metadata::template properties<T>());
        return count;
    }

    template<class T> void save_record(A& a, const T& value)
    {
        object_header header{std::string(record_info<T>::type_id), record_info<T>::version, field_count<T>()};
        archive_traits<A>::write_object(a, header, [&](A& object)
        {
            ...
            std::apply([&](const auto&... fields)
            {
                auto field = [&](const auto& f)
                {
                    if constexpr (requires { f.member(); })
                    {
                        ...
                        {
                            archive_traits<A>::write_field(object, key,
    ...
    template<class T> void load_record(A& a, T& value)
    {
        object_header expected{std::string(record_info<T>::type_id), record_info<T>::version, field_count<T>()};
        archive_traits<A>::read_object(a, expected, [&](A& object, const object_header& actual)
        {
            if (actual.version != expected.version)
                throw serialization_error(error_code::unsupported_version, "Record version mismatch");
            ...
            std::apply([&](const auto&... fields)
            {
                auto field = [&](const auto& f)
                {
                    if constexpr (requires { f.member(); })
                    {
                        ...
                        at("." + std::string(key.name), [&]
                        {
                            const auto presence = archive_traits<A>::read_field(object, key,
                                [&](A& child) { load(child, value.*f.member()); });
                            if (presence == field_presence::missing)
                                throw serialization_error(error_code::missing_field, "Required field is missing");
                        });
                    }
                };
                (field(fields), ...);
    ...
    template<class T> void save_value(A& a, const T& value)
    {
        using V = std::remove_cv_t<T>;
        constexpr bool codec = requires { type_codec<V>::save(a, value, *this); };
        static_assert(!(codec && !core_detail::builtin_codec<V> &&
        ...
        else if constexpr (HasMetadata<Metadata, V>) save_record(a, value);
        else if constexpr (std::is_enum_v<V>) save(a, static_cast<std::underlying_type_t<V>>(value));
        else if constexpr (std::is_arithmetic_v<V> || std::is_same_v<V, std::string>)
        {
            if constexpr (std::is_same_v<V, std::string>)
                if (value.size() > options_.max_string_bytes)
                    throw serialization_error(error_code::size_mismatch, "String exceeds byte limit");
        ...
        else if constexpr (codec) type_codec<V>::save(a, value, *this);
        else static_assert(core_detail::unsupported<T>, "No serialization metadata or type_codec for type");
    }

    template<class T> void load_value(A& a, T& value)
    {
        using V = std::remove_cv_t<T>;
        ...
        else if constexpr (HasMetadata<Metadata, V>) load_record(a, value);
        ...
        else if constexpr (codec) type_codec<V>::load(a, value, *this);
        else static_assert(core_detail::unsupported<T>, "No deserialization metadata or type_codec for type");
    }
...
public:
    ...
    template<OutputArchive A, class T> void save(A& a, const T& value) const
    {
        context<A, MetadataProvider> ctx(options_);
        ctx.save(a, value);
    }
    template<InputArchive A, class T> void load(A& a, T& value) const
    {
        context<A, MetadataProvider> ctx(options_);
        ctx.load(a, value);
    }
    template<OutputArchive A, class T>
    void save(A& a, const T& value, context<A, MetadataProvider>& ctx) const { ctx.save(a, value); }
    template<InputArchive A, class T>
    void load(A& a, T& value, context<A, MetadataProvider>& ctx) const { ctx.load(a, value); }
private:
    operation_options options_;
};
...
public:
    ...
    template<class Derived> void add(std::string id)
    {
        static_assert(std::derived_from<Derived, Base> && std::has_virtual_destructor_v<Base>,
            "Polymorphic registration requires public inheritance and a virtual base destructor");
        if (frozen_) throw serialization_error(error_code::invalid_registry, "Registry is frozen");
        const auto type = std::type_index(typeid(Derived));
        if (id.empty() || by_id_.contains(id) || by_type_.contains(type))
            throw serialization_error(error_code::duplicate_type, "Empty or duplicate polymorphic type ID/type");
        entry e;
        e.id = id;
